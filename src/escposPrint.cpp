...
BOOL PrintRawDataByLpt(string devicePath, char *meg, size_t size,
                       PrintResult *result) {
  DWORD dwWrite;
  HANDLE hLPT = CreateFile(devicePath.c_str(), GENERIC_READ | GENERIC_WRITE, 0,
  ...
  BOOL b = WriteFile(hLPT, meg, (DWORD)size, &dwWrite, NULL);
  if (!b) {
    SetPrintResult(result, FALSE, GetLastError());
    return FALSE;
  }
...
HANDLE InitPort(PrintDevice &device) {
  HANDLE handle =
      CreateFile(device.Port.c_str(), GENERIC_READ | GENERIC_WRITE, 0, NULL,
                 OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

  if (handle == INVALID_HANDLE_VALUE) {  // 打开端口失败
    return FALSE;
  } else {
    // 设置端口缓冲
